...
- `scripts/cute_slime.gd` — `CuteSlime`: the first designed enemy (strong,
  ...
  and all jokers (the Sticky effect); her strategy oozes slimed cards next to
  jokers to seal them in slime, and she alone moves slimed cards freely
...
"slime strategy" that oozes slimed cards next to jokers to seal them in — the
smart AI understands the slime and never plans a move that would drag a cluster
it didn't mean to. Enemies live in `scripts/enemy.gd` (+ `cute_slime.gd`); the
rogue ladder picks one at random each round (only the slime exists for now).
